...
class FCBlock(nn.Module):
 ...
 # %%
class GLN(nn.Module):
    def __init__(self, in_features, in_envs, num_classes, num_residual_blocks=2, m1=2, m2=2, C=4, num_predictor_blocks=4, p=0.2):
        super().__init__()        
        self.m1 = m1
        self.m2 = m2
        ...
        Output2 = math.ceil(Output1/(2*m2)**num_residual_blocks)*C # we flatten after the last block
        out_envs = min(in_envs,10) # the output of the Linear Layer of Tab input should be smaller than the input and max 10
        Output3 = Output2 + out_envs # after concatenation of Genotype data with Tabular Data
        self.TabLayer = nn.Linear(in_features=in_envs, out_features=out_envs)
        self.FCLayers = self.make_predictorLayers(in_features=Output3)
    ...
    def forward(self,SNP,env):
        x = F.one_hot(SNP.long(), num_classes=4)
        x = x.transpose(-1,-2)
        x = self.LCL0(x)
        x = self.LCLayers(x)
        x = x.flatten(1)

        y = self.TabLayer(env.float())
